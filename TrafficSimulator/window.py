...
class Window:
    def __init__(self, simulation):
        ...
        pygame.display.set_caption('GeneticSem')
        pygame.display.flip()
        pygame.font.init()
    ...
    def _draw_signals(self) -> None:
        # Dibuja cada grupo: verde si esa entrada está habilitada en la fase actual, rojo si no.
        for signal in self._sim.traffic_signals:
            mask = signal.current_cycle  # tupla de bools, p.ej. (True, False, False, False)
            for gi, group in enumerate(signal.roads):
                color = (0, 255, 0) if mask[gi] else (255, 0, 0)
                for road in group:
                    # Dibuja la “luz” cerca del final de la vía de entrada
                    a = 0.02  # 0=exacto en la intersección; 0.02=un poco antes
                    position = (
    ...
    def _draw_hud(self) -> None:
        # HUD
        text_fps = self._text_font.render(f'FPS: {self._sim.t:.0f}', True, (0, 0, 0), (255, 255, 255))
        text_time = self._text_font.render(f'Time: {self._sim.t:.1f}', True, (0, 0, 0), (255, 255, 255))
        text_vehicles = self._text_font.render(f'Vehicles: {self._sim.n_vehicles_on_map}', True, (0, 0, 0),
                                               (255, 255, 255))
        text_average_wait_time = self._text_font.render(f'Average wait time: {self._sim.current_average_wait_time:.2f}s',
                                                        True, (0, 0, 0), (255, 255, 255))
        
        # Add scenario info
        scenario_name = getattr(self._sim, 'scenario_name', 'Unknown')
        num_nodes = getattr(self._sim, 'num_nodes', 0)
        num_roads = getattr(self._sim, 'num_roads', len(self._sim.roads))
        text_scenario = self._text_font.render(f'Scenario: {scenario_name} | Nodes: {num_nodes} | Roads: {num_roads}',
                                               True, (0, 0, 0), (255, 255, 255))

        self._screen.blit(text_fps, (0, 0))
        self._screen.blit(text_time, (0, 20))
        self._screen.blit(text_vehicles, (0, 40))
        self._screen.blit(text_average_wait_time, (0, 60))
        self._screen.blit(text_scenario, (0, 80))
