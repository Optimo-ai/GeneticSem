...
def _unwrap_config(config):
    # Aceptar dicts como los guardados por GA: {"best_config":[...], ...}
    if isinstance(config, dict):
        if "best_config" in config and isinstance(config["best_config"], list):
            return config["best_config"]
        # fallback: cualquier valor lista
...
def _get_build_callable(scenario_id: int):
    ...
    return _BUILDERS[scenario_id]()  # devuelve build(config, render)
...
def make_sim_factory(scenario_id: int) -> Callable[[list, bool], "Simulation"]:
    ...
    def sim_factory(config, render: bool = False):
        cfg = _unwrap_config(config)
        return build(config=cfg, render=render)
    return sim_factory

def get_phases_per_signal(scenario_id: int) -> List[int]:
    build = _get_build_callable(scenario_id)  # builder con atributo phases_per_signal
    phases = getattr(build, "phases_per_signal", None)
    return list(phases) if phases else [4]

def get_scenario_name(scenario_id: int) -> str:
    return SCENARIOS.get(scenario_id, "Unknown")
